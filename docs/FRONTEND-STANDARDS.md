...
## 9.5 演示与引导机制（2026-08-28 新增）

- **入场生长动画**：画布挂载/预设切换时，曲线 1.25s 从左生长至全宽（easeOutCubic，起点 35% 成熟度避免空屏）——这是站点"签名瞬间"。
- **自动演示**：参数演示（3.2s 起→峰→回，结束归位默认值）与切点巡航（4.2s 沿曲线扫描，尾部留淡色轨迹）；任意交互（pointerdown/预设/表达式）即时打断；按钮高亮呼吸动画示意"进行中"。
- **快捷键**：1-7 切预设、R 重置、D/T 演示；输入框聚焦时不触发。
- **首次引导**：三条顺序气泡（调参/设切点/演示），sessionStorage 每会话一次，可关闭。
- 动画物理：均尊重 prefers-reduced-motion；Canvas 动画走 rAF 直绘（drawRef 模式），不触发 React 重渲染；参数推进 25fps 节流。

## 9.6 交互回归测试（2026-08-28 新增，38 项断言）

- scripts/interact-test.mjs：CDP 驱动 headless Chrome 真实输入事件逐项点击全部控件：
  顶栏（主题/手写板）、侧边栏（学段/搜索/折叠/免费体验切换）、函数实验（表达式错误态/滑块/开关/画布点击/拖拽/缩放/演示/快捷键/挑战）、单位圆（θ 滑块/自动旋转/归零/点击设角）、圆锥曲线（类型切换/参数/挑战）
- 断言原则：状态必须真实变化（画布用全文散列、URL 用搜索参数、按钮用 aria 状态）
- 每个 UI 改动合入前必须全绿（历史教训：拖拽平移不重绘、URL 防抖被动画饿死、初中门控死代码，均由此测试抓出）
- 失败诊断：断言 extra 写 scripts/results.txt，避免 stdout/stderr 混杂
